...
NSA5::rho_3::rho_3(const ::NSA4::builder_set& block_builder_set)
...
NSA5::rho_3::impl::impl(const ::NSA4::builder_set& block_builder_set)
...
cmplx_vec NSA5::rho_3::do_eval(int m1, int m2) const
{
    auto b_rho_3 = cmplx_vec(2);

    b_rho_3[0] = pimpl->eval_first_component(m1, m2);
    b_rho_3[1] = pimpl->eval_second_component(m1, m2);
...
cmplx_dbl NSA5::rho_3::impl::eval_first_component(int m1, int m2) const
{
    auto result = cmplx_dbl{};

    result = ( g_rho.eval(2*m1, 2*m2+1)
	       + simp_quad_rho_rho.eval(0, m1-1, 2*m1, 2*m2+1) );

    if (m1 != 0)
    {
	result += ( (1.0 / 3.0) * ( M_rho_rho_1.eval(2*m1, 2*m2+1, 2*m1-2)
				    * y_rho[2*m1-2] )
...
cmplx_dbl NSA5::rho_3::impl::eval_second_component(int m1, int m2) const
{
    auto result = cmplx_dbl{};

    result = ( g_rho.eval(2*m1+1, 2*m2+1)
	       + simp_quad_rho_rho.eval(0, m1-1, 2*m1+1, 2*m2+1)
	       - (1.0 / 12.0) * ( M_rho_rho_2.eval(2*m1+1, 2*m2+1, 2*m1)
				  * y_rho[2*m1-1] ) );

    if (m1 != 0)
    {
	result += ( (1.0 / 3.0) * ( M_rho_rho_1.eval(2*m1+1, 2*m2+1, 2*m1-2)
				    * y_rho[2*m1-2] )
