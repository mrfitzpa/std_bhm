...
cmplx_vec NSA5::K_4::do_eval(int m1, int m2) const
{
    ...
    const auto& M_rho_K_6 = pimpl->M_rho_K_6;
    
    auto C_K_4 = cmplx_vec(4);

    C_K_4[0] = M_rho_K_4.eval(2*m1, 2*m2+1, 2*m1) / 3.0;
    C_K_4[1] = 0.0;
    C_K_4[2] = (M_rho_K_4.eval(2*m1+1, 2*m2+1, 2*m1) / 2.0
		  + M_rho_K_6.eval(2*m1+1, 2*m2+1, 2*m1) / 2.0);
    C_K_4[3] = (M_rho_K_4.eval(2*m1+1, 2*m2+1, 2*m1+1) / 6.0
		  + M_rho_K_6.eval(2*m1+1, 2*m2+1, 2*m1) / 4.0);
