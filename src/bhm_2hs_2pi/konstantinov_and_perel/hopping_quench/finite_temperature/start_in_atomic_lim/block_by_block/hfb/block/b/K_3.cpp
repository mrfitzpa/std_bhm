...
{
    auto b_K_3 = cmplx_vec(2);

    b_K_3[0] = pimpl->eval_first_component(m1, m2);
    b_K_3[1] = pimpl->eval_second_component(m1, m2);
...
cmplx_dbl NSA5::K_3::impl::eval_first_component(int m1, int m2) const
{
    const auto mL = std::min(window_index, m2);

    auto result = cmplx_dbl{};

    result = ( g_K.eval(2*m1, 2*m2+1)
    ...
    if (m1 != 0)
    {
	result += ( (1.0 / 3.0) * ( M_rho_K_4.eval(2*m1, 2*m2+1, 2*m1-2)
				    * y_K[2*m1-2] )
...
cmplx_dbl NSA5::K_3::impl::eval_second_component(int m1, int m2) const
{
    const auto mL = std::min(window_index, m2);

    auto result = cmplx_dbl{};

    result = ( g_K.eval(2*m1+1, 2*m2+1)
	       ...
	       + (1.0 / 6.0) * ( M_K_rho_1.eval(2*m1+1, 2*m2+1, 2*mL+1)
				 * y_rho[2*mL+1] )
	       - (1.0 / 12.0) * ( M_rho_K_5.eval(2*m1+1, 2*m2+1, 2*m1)
				  * y_K[2*m1-1] ) );

    if (m1 != 0)
    {
	result += ( (1.0 / 3.0) * ( M_rho_K_4.eval(2*m1+1, 2*m2+1, 2*m1-2)
				    * y_K[2*m1-2] )
